...
-- Skip Player
function SkipPlayer()
    print("Đã bỏ qua người chơi", "warning")
...
function target() 
    pcall(function()
        print("Đang tìm mục tiêu mới...", "info")
...
game:GetService("CoreGui").RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(child)
    if not hopserver and child.Name == 'ErrorPrompt' and child:FindFirstChild('MessageArea') and child.MessageArea:FindFirstChild("ErrorFrame") then
        print("Tuan Anh Auto Bounty | Đang vào lại!")
...
function wEarn(targ, earn, total) 
    if getgenv().Setting.Webhook.Enabled and getgenv().killed then
        ...
        local data = {
            ...
            ["embeds"] = {
                {
                    ["title"] = "**Chú Roblox hub | Auto Bounty**",
    ...
    ["embeds"] = {
        {
            ...
            ["fields"] = {
                ...
                {
                    ["name"] = "Ty for test scr",
                    ["value"] = "Sẽ có nhiều script hone trong tương lai",
                    ["inline"] = false
                }
            }
        }
