...
local function NotificacaoNightMystic(titulo, mensagem)
    local success = pcall(function()
        ...
        local LogoID = "rbxassetid://100306458933414"

        local ScreenGui = Instance.new("ScreenGui")
        ScreenGui.Name = "Nak Hub"
...
NotificacaoNightMystic("Nak Hub", "Naknohack")

-- ========================================
-- SAVE SYSTEM (Optimized)
-- ========================================
local HttpService = Services.HttpService
local FolderName = "Nak Hub"
...
Sea = World1 or World2 or World3 or Dungeon
...
Discord:AddDiscordInvite({
    Name = "Nak Hub",
    Description = "Join",
    Logo = "rbxassetid://100306458933414",
...
local MeleeCoords = {
    ...
    ["Dragon Talon"] = {
        Key = "BuyDragonTalon",
        Pos = World3 and CFrame.new(5661, 1211, 865)
    },
    ["Electric Claw"] = {
        Key = "BuyElectricClaw",
        Pos = World3 and CFrame.new(-10371, 331, -10131)
    },
    ["Godhuman"] = {
        Key = "BuyGodhuman",
        Pos = World3 and CFrame.new(-13776, 334, -9879)
    },
    ["Sanguine Art"] = {
        Key = "BuySanguineArt",
        Pos = World3 and CFrame.new(-16353, 160, 99)
    }
}

-- LẤY LIST MELEE HỢP LỆ
local function GetMelee()
    local t = {}
    for k,v in pairs(MeleeCoords) do
        if v.Pos then
            table.insert(t,k)
        end
    end
...
-- FIX DEFAULT KHÔNG LỖI
local meleeList = GetMelee()
local SelectedMelee = meleeList[1]
...
Status:AddSection("Check Stock Fruits")
...
local Stock = Status:AddParagraph({
...
Others:AddToggle({
    Name = "Tự động nhận nhiệm vụ câu",
...
Others:AddToggle({
    Name = "Hoàn thành nhiệm vụ tự động",
...
-- 8. AUTO SPAM SKILL Z
Others:AddToggle({
    Name = "Tự động dùng skills Z câu",
...
Others:AddToggle({
 ...
 Others:AddToggle({
    Name = "Farm tất cả Boss trong sever",
    Description = "",
    Default = false,
    Callback = function(v)
        _G.FarmAllBoss = v
...
spawn(function()
 ...
 Others:AddToggle({
    Name = "Tự động dựng lại khi có vật phẩm",
    Description = "",
...
Event:AddToggle({
    Name = "Auto Đánh Pirate GrandBrigade",
    Description = "",
    Default = false,
    Callback = function(I)
        _G.PGB = I;
...
Event:AddToggle({
    Name = "Auto đánh Fish Crew Member",
    Description = "",
    Default = false,
    Callback = function(I)
        _G.MobCrew = I;
...
Event:AddToggle({
    Name = "Auto Đánh Haunted Crew Member",
    Description = "",
    Default = false,
    Callback = function(I)
        _G.HCM = I;
...
--// ================= TAB =================
local SPYING = Event:AddParagraph({
    Title = "Chip Đã Mua",
    Content = "Loading..."
})
    ...
    while wait(0.2) do
        pcall(function()
            local result = game:GetService("ReplicatedStorage")
                ...
                :WaitForChild("CommF_")
                :InvokeServer("InfoLeviathan", "1")

            local count = string.match(result, "%d+")

            if count then
                SPYING:SetDesc(" Spy Leviathan : " .. tostring(count))
...
Event:AddToggle({
    Name = "Auto tìm Frozen Dimension",
    Description = "",
    Default = GetSetting("FindFrozen_Save", false),
    Callback = function(I)
        _G.AutoFindFrozen = I
        _G.SaveData["FindFrozen_Save"] = I
        SaveSettings()
    end
})
	...
	while wait() do
		if _G.AutoFindFrozen then
			pcall(function()
				...
				local Frozen = workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension", true)
				...
				if not Frozen then
					...
					if not Boat then
						...
						if (BuyPos.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then
							replicated.Remotes.CommF_:InvokeServer("BuyBoat", _G.SelectedBoat)
						end

					else
						if not plr.Character.Humanoid.Sit then
							local Seat = Boat.VehicleSeat.CFrame * CFrame.new(0,1,0)
							_tp(Seat)

						else
							-- ⚙️ bắt đầu di chuyển
							local currentX = -10000000
							local currentZ = 37016.25

							repeat
								wait()

								local hasSea = CheckEnemiesBoat() or CheckTerrorShark() or CheckPirateGrandBrigade()

								if hasSea then
									-- 🚀 bay lên
									_tp(CFrame.new(currentX, 150, currentZ))
									wait()

									-- ➡️ bay ngang trên cao
									currentX = currentX - 1000
									_tp(CFrame.new(currentX, 150, currentZ))
									wait()

									-- ⬇️ hạ xuống
									_tp(CFrame.new(currentX, 31, currentZ))
								...
								-- ✅ check Frozen liên tục
								Frozen = workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension", true)

							until not _G.AutoFindFrozen
							or Frozen
					...
					-- ✅ nếu thấy sẵn thì bay lên trên đảo
					_tp(Frozen.CFrame * CFrame.new(0, 300, 0))
					plr.Character.Humanoid.Sit = false
					_G.AutoFindFrozen = false
				end
			...
			local function Attack(part)
				repeat task.wait(0.1)
					...
					if player.Character and part then
						...
						if hrp then
							hrp.CFrame = part.CFrame * CFrame.new(0,20,0)
						end

						UseSkill()
					end

				until not part.Parent or not _G.Leviathan1
			end


			while _G.Leviathan1 do
				task.wait(1)
				...
				if SeaBeasts then
					...
					if Levi then
						...
						for _,v in pairs(Levi:GetChildren()) do
							if string.find(v.Name,"Segment") or string.find(v.Name,"Body") then
								Attack(v)
							end
						end
...
--// ================= TAB =================
Event:AddToggle({
    Name = "Auto lái thuyền về Tiki Outpost",
    Description = "",
    Default = GetSetting("AutoTikiBoat_Save", false),
    Callback = function(I)
        _G.AutoTikiBoat = I
        _G.SaveData["AutoTikiBoat_Save"] = I
        SaveSettings()
    end,
})

--// ================= LOGIC =================
if not _G.TikiBoatLoop then
    _G.TikiBoatLoop = true

    v505.RenderStepped:Connect(function()
        if not _G.AutoTikiBoat then
            return
        end
        ...
        for _, boat in pairs(v507.Boats:GetChildren()) do
            ...
            if s and s.Occupant == humanoid then
                seat = s
                break
            end
        end
        ...
        if seat then
            ...
            -- ✅ auto chạy không cần W
            seat.Throttle = 1
            seat.Steer = 0

            local target = Vector3.new(-16217.7568, 9.1267, 446.0653)
            local current = seat.Position

            local direction = (target - current).Unit
            local move = direction * seat.MaxSpeed * v505.RenderStepped:Wait()

            seat.CFrame = seat.CFrame + move
            seat.CFrame = CFrame.new(seat.Position, target)

            -- tới nơi thì dừng
            if (seat.Position - target).Magnitude < 120 then
                _G.AutoTikiBoat = false
                _G.SaveData["AutoTikiBoat_Save"] = false
                SaveSettings()
                seat.Throttle = 0
            end
        end
...
--// ================= TAB =================
Event:AddToggle({
    Name = "Auto lái thuyền về Hydra",
    Description = "",
    Default = GetSetting("AutoHydraBoat_Save", false),
    Callback = function(I)
        _G.AutoHydraBoat = I
        _G.SaveData["AutoHydraBoat_Save"] = I
        SaveSettings()
    end,
})

--// ================= LOGIC =================
if not _G.HydraBoatLoop then
    _G.HydraBoatLoop = true

    v505.RenderStepped:Connect(function()
        if not _G.AutoHydraBoat then
            return
        end
        ...
        if seat then
            ...
            local target = Vector3.new(5193.9375, -0.0469, 1631.5783)
            local current = seat.Position
            ...
            -- tới nơi thì dừng
            if (seat.Position - target).Magnitude < 120 then
                _G.AutoHydraBoat = false
                _G.SaveData["AutoHydraBoat_Save"] = false
                SaveSettings()
                seat.Throttle = 0
            end
        end
...
Event:AddToggle({
    Name = "Auto tìm đảo Kitsune Island",
...
Event:AddToggle({
	Name = "Auto Di chuyển đến đền Kitsune kích hoạt",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.tweenShrine = I;
...
Event:AddToggle({
    Name = "Auto Thu thập Azure Ember",
    Description = "",
    -- 1. Carrega o estado salvo ou false por padrão
...
Event:AddToggle({
	Name = "Auto tìm đảo Mirage Island",
...
Event:AddToggle({
	Name = "Auto bay đến điểm cao nhất",
...
Event:AddToggle({
	Name = "Auto Bay đến chỗ Người bán trái Blox nâng cao",
...
Event:AddToggle({
	Name = "Auto Nhặt rương ở đảo Mirage",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.FarmChestM = I;
...
Event:AddToggle({
    Name = "Auto Chế tạoVolcanic Magnet",
    Description = "Bật chế độ tự động Auto cày nguyên liệu và chế tạo nam châm núi lửa, và dừng lại khi bạn đã có 1 nam châm núi lửa",
    Default = false,
    Callback = function(I)
        _G.CraftVM = I;
    end,
...
Event:AddToggle({
    Name = "Auto Tìm Prehistoric Island",
    Description = "",
...
Event:AddToggle({
    Name = "Auto Farm Đảo Prehistoric Island",
    Description = "",
    -- 1. Carrega o estado salvo
...
-- 3. TOGGLE: Auto Reset
Event:AddToggle({
    Name = "Auto Reset Khi hoàn thành đảo",
    Description = "",
...
Maestry:AddToggle({
    Name = "Auto Farm Thông thạo Fruit",
    Description = "",
...
Race:AddToggle({
	Name = "Auto Di chuyển đến cửa tộc",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.TPDoor = I;
...
Race:AddToggle({
	Name = "Auto Hoàn thành Trial Tộc",
	Description = "",
	Default = false,
	Callback = function(I)
...
Race:AddToggle({
	Name = "Auto Kill Player Trong Trial",
...
Dojo:AddToggle({
	Name = "Auto Trial Biến hình Tộc draco",
...
Dojo:AddToggle({
	Name = "Bay",
	Description = "Dùng để khi Auto Trên lỗi dùng chay này thế",
	Default = false,
	Callback = function(v)
		_G.Fly = v
	end,
...
EspPly = function()
    for _, e in next, game.Players:GetChildren() do
        pcall(function()
            if not isnil(e.Character) then
                if PlayerEsp then
                    if not isnil(e.Character.Head) and not e.Character.Head:FindFirstChild("NameEsp" .. Number) then
                        ...
                        K.TextColor3 = Color3.new(0, 1, 0)
                    else
                        e.Character.Head["NameEsp" .. Number].TextLabel.Text =
                            e.Name .. " | Lv. " .. (e.Data.Level.Value)
                    end
                else
                    if e.Character.Head:FindFirstChild("NameEsp" .. Number) then
                        e.Character.Head["NameEsp" .. Number]:Destroy()
                    end
                end
            end
        end)
            ...
            if DevilFruitESP then
                if string.find(e.Name, "Fruit") then
                    if not e.Handle:FindFirstChild("NameEsp" .. Number) then
                        ...
                        e.Handle["NameEsp" .. Number].TextLabel.Text =
                            "[" .. e.Name .. ("]\n" ..
                            (G5(((game:GetService("Players")).LocalPlayer.Character.Head.Position - e.Handle.Position).Magnitude / 3) .. " M"))
                    end
                end
...
-- ISLAND ESP
_G.ESPColor = Color3.fromRGB(0, 140, 255)
...
getgenv().WalkSpeedValue = getgenv().WalkSpeedValue or 26
SpeedEnabled = false

function ApplyWalkSpeed()
    local humanoid = l_LocalPlayer_18.Character
        and l_LocalPlayer_18.Character:FindFirstChild("Humanoid")
    if humanoid then
        humanoid.WalkSpeed = SpeedEnabled and getgenv().WalkSpeedValue or 16
    end
end
--==================================================
...
Get:AddToggle({
 Name = "Auto Lấy CDK [Nhiệm vụ cuối cùng]",
    Description = "",
    -- 1. Carrega o estado salvo ou false por padrão
...
Get:AddToggle({
    ...
    Callback = function(Value)
        _G.Tp_LgS = Value
        _G.SaveData["TpLegendarySword_Save"] = Value
        SaveSettings()
...
Get:AddToggle({
 Name = "Auto Lấy Áo choàng Hải Quân",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.MarinesCoat = I;
...
Get:AddToggle({
 Name = "Auto Lấy Áo choàng Gai Đen",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.BlackSpikey = I;
...
Get:AddToggle({
 Name = "Auto Lấy phụ kiện vòng hoa",
	Description = "",
	Default = false,
	Callback = function(I)
		_G.AutoKilo = I;
...
Fruit:AddToggle({
    ...
    Description = "Đợi 10s nếu trong sv có người khác đang Raid thì chức năng này ko tự vô đc mn thông cảm nha",
    Default = false,
    Callback = function(I)
        _G.Auto_StartRaid = I;
    end,
...
Fruit:AddToggle({
	Name  = "Tự động đánh quái+qua đảo",
    Description = "",
    Default = false,
    Callback = function(I)
        _G.Raiding = I
...
_G.Fast_Delay = _G.Fast_Delay or 0.5
_G.GoingExit = false
_G.DeathPause = false
_G.DungeonBring = true
...
local function GoNextFloor()
	if not _G.Dungeonh or _G.GoingExit or not IsDungeonWorld() then return end
	_G.GoingExit = true
	StartBring = false
...
task.spawn(function()
	while task.wait(0.45) do
		if not _G.Dungeonh or _G.GoingExit or _G.DeathPause then continue end
		...
		for _, v in pairs(workspace.Enemies:GetChildren()) do
			if v:FindFirstChild("Humanoid")
				...
				and v.Humanoid.Health > 0
				and not string.find(v.Name, "Shadow") then
				alive = true
				break
			end
		end
...
Setting:AddButton({ Name = "Xóa Sương mù", Description = "", Callback = function()
...
NotificacaoNightMystic("Nak Hub", "✅ Todos os sistemas carregados!")
